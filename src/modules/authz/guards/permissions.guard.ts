  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      const actorPermissions =
        await this.authzService.resolvePermissions(actor);

      // Validar que el actor tenga todos los permisos requeridos
      // Soporta: permisos exactos, module.*, y *
      const hasAllPermissions = requiredPermissions.every((perm) =>
        this.authzService.hasPermission(actorPermissions, perm),
      );
      ...
      this.logger.error(
        `Permission check failed for ${actor.actorType}:${actor.actorId}: ${error.message}`,
        error.stack,
      );
